...
try
    ...
    for i = 0:options.getLength-1
        ...
        if (strcmpi(name, 'imgDir')) imgDir = char(options.item(i).getAttribute('value')); end
        if (strcmpi(name, 'lblDir')) lblDir = char(options.item(i).getAttribute('value')); end
...
for i = 0:options.getLength-1
    ...
    if (strcmpi(name, 'outputDir'))
        ...
        inputDir = [outputDir 'fold' num2str(fold) '/'];
    ...
    if (strcmpi(name, 'modelsDir'))
        ...
        if~isdir(modelsDir) mkdir(modelsDir); end
    end
    if (strcmpi(name, 'auxFeatureDir'))% && stage~=1)
        auxFeatureDir = char(options.item(i).getAttribute('value'));
        %auxFeatureDir = [auxFeatureDir 'fold' num2str(fold) '/stage' num2str(stage)];
        auxFeatureDir = [auxFeatureDir 'fold' num2str(fold)];
